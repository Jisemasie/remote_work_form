...
export async function checkPasswordChangeRequired(userId: number): Promise<boolean> {
    try {
        ...
        const result = await executeDataRequest('sp_CheckPasswordChangeRequired', params, true);
        return result && result.length > 0 ? result[0].must_change_password : false;
    } catch (error) {
...
export async function incrementFailedLoginAttempts(username: string) {
    try {
        const params: InputParam[] = [
            { key: 'username', value: username }
        ];
...
export async function changeUserPassword(
    ...
    try {
        ...
        const params: InputParam[] = [
            ...
            { key: 'new_password', value: newPassword }
        ];

        const result = await executeInsertUpdateRequest('sp_ChangeUserPassword', params, true);
        
        if (result && result.length > 0) {
            const response = result[0];
            if (response.success) {
                await logUserActivity(userId, 'PASSWORD_CHANGED', 'User changed password');
